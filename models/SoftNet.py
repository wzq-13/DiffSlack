...
import tqdm
import globalvar
from utils.utils import visualize_data_batch, check_polygon_intersection, get_rect_points_vectorized, visualize_data_batch_paper, path_smoothness
...
class PINN_Trainer:
    def __init__(self, config, train_loader, val_loader, test_loader=None, save_dir=None, load_dir=None, log_dir=None, model='MLP'):
        ...
        self.model = create_model(self.config, model_type=self.model_type, device=DEVICE)
    ...
    def train(self, begin_epoch: int = 0):
        ...
        # self.test_visualization(save_path='imgs')
        # return
        ...
        for epoch in range(begin_epoch, num_epochs):
            ...
            if (epoch + 1) % self.config['visual_step'] == 0:
                self.test_visualization(save_path='imgs/Ablation')
            if (epoch + 1) % self.config['save_step'] == 0:
                self._save_model(epoch=epoch)
        # test_metrics = self.evaluate(self.test_loader)
        # print(f'=== Test Loss: {test_metrics["total_loss"]:.4f}, obj Loss: {test_metrics["obj_loss"]:.4f}, Constraint Residual: {test_metrics["constraint_residuals"]:.4f}')
    ...
    # def path_clean(self, path: torch.Tensor) -> torch.Tensor:
    #     '''
    #     path : (N, 2)
    #     '''
    #     new_path = [path[0].cpu().numpy()]
    #     for i in range(1, path.shape[0]):
    #         prev_point = new_path[-1]
    #         curr_point = path[i].cpu().numpy()
    #         vec1 = curr_point - prev_point
    #         if np.linalg.norm(vec1) > 0.2:  # Threshold distance
    #             new_path.append(curr_point)
    #     return torch.tensor(new_path, dtype=torch.float32)
        ...
        with torch.no_grad():
            for i, X_batch in enumerate(self.test_loader):
                for key in X_batch:
                    X_batch[key] = X_batch[key].to(DEVICE, non_blocking=True)
                Y_pred = self.model(X_batch)
                trajectories = Y_pred.view(Y_pred.size(0), -1, 7)[:, :, :2]
                visualize_data_batch(X_batch, trajectories, save_path=save_path)
