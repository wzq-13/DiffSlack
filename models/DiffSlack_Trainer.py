...
class APF_Hard_Trainer:
    def __init__(self, config, train_dataset, val_dataset, test_dataset=None, save_dir=None, load_dir=None, log_dir=None):
        ...
        self.adanp = AdaNP(n_outputs=80, n_constraints=200, max_depth=self.config['max_depth'], tol=self.config['inference_tol'])
        self.adanp_test = AdaNPTest(max_depth=self.config['max_depth'], tol=self.config['inference_tol'])
        ...
        with torch.no_grad():
            ...
            for X_batch in data_loader:
                for key in X_batch:
                    X_batch[key] = X_batch[key].to(DEVICE, non_blocking=True)
                Y_pred = self.model(X_batch)
                Y_proj, actual_depth =  self.adanp_test(X_batch, Y_pred, self.constraint_func_stage2)
                warm_num -= 1
                if warm_num <=0:
                    break
